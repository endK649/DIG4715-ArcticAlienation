...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void MyInput()
    {
        ...
        if (Input.GetKey(crouchKey))
        {
            ...
            if (!hasAppliedForce) // Downward force only applies on first crouchKey press and not continuous
            {
                ...
                rb.AddForce(rb.velocity, ForceMode.Impulse); // Preserve existing momentum
        ...
        // when to slide
        if (Input.GetKey(crouchKey) && (horizontalInput != 0 || verticalInput != 0) && (state == MovementState.sprinting || rb.velocity.magnitude > walkSpeed) && !sliding)
        {
            StartSlide();
        }
    ...
    private void Jump()
    {
        ...
        Vector3 preservedVelocity = new Vector3(rb.velocity.x, 0f, rb.velocity.z);

        // Reset only Y velocity for consistent jump height
        rb.velocity = new Vector3(preservedVelocity.x, jumpForce, preservedVelocity.z);
